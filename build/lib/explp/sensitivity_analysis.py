...
# Now for the Mixed-Integer LP problems:
# Similar to Simple LP
# But Slightly changed for rounding to integer
    
    
# optimal varible sensitivity analysis function for MILP
# By changing optimal solution up and down 1 unit and -100% to +100%, rounding to the nearest integer

def MI_Optimal_Var(optimal_solution_Branch_and_Bound):
    
    global MI_table_var_changes
    
    MI_optimal_solution = copy.deepcopy(optimal_solution_Branch_and_Bound)
    constraint_value = copy.deepcopy(Deep_Copy_constraint[1])
    
    optimal_objective_value = MI_optimal_solution[0]
    optimal_variable_value = MI_optimal_solution[1]
    optimal_slack_value = MI_optimal_solution[2]
    
    # introduce a spacing for table
    
    spacing = '...'
    
    # record which constraint is violated if optimal solution + / - 1 & + / - 10-100%, rounding to the nearest integer
    ...
    for variable_counter in range(len(variable_names)):
        ...
        for constraint_counter in range(len(constraint_names)):
            ...
            if constraint_value[constraint_counter][variable_counter]*round(optimal_variable_value[variable_counter]*(-1)) > optimal_slack_value[constraint_counter]:
                temp_list_m100.append(constraint_names[constraint_counter])
            
            if constraint_value[constraint_counter][variable_counter]*round(optimal_variable_value[variable_counter]*(-0.5)) > optimal_slack_value[constraint_counter]:
                temp_list_m50.append(constraint_names[constraint_counter])
  
            if constraint_value[constraint_counter][variable_counter]*round(optimal_variable_value[variable_counter]*(-0.25)) > optimal_slack_value[constraint_counter]:
                temp_list_m25.append(constraint_names[constraint_counter])
            
            if constraint_value[constraint_counter][variable_counter]*round(optimal_variable_value[variable_counter]*(-0.1)) > optimal_slack_value[constraint_counter]:
                temp_list_m10.append(constraint_names[constraint_counter])
    ...
    col_var_changes = ['Optimal Solution - 1', 
                       ...
                       'Optimal Solution + 1', 
                       spacing, 
                       'Optimal Solution - 100% (rounding to integer)', 
                       'Optimal Solution - 50% (rounding to integer)', 
                       'Optimal Solution - 25% (rounding to integer)', 
                       'Optimal Solution - 10% (rounding to integer)', 
                       'Optimal Solution', 
                       'Optimal Solution + 10% (rounding to integer)', 
                       'Optimal Solution + 25% (rounding to integer)', 
                       'Optimal Solution + 50% (rounding to integer)', 
    ...
    for row_counter in range(len(variable_names)):
        ...
        row_var_changes.append(spacing)
        row_var_changes.append('Value of ' + variable_names[row_counter] + ' (rounding to integer)')
        row_var_changes.append('Value of ' + obj_names[0])
        row_var_changes.append('Violated Constraints')
        row_counter += 1
    ...
    MI_table_var_changes = pd.DataFrame(columns = col_var_changes, index = row_var_changes).fillna('')
    
    fill_counter = 0
    
    for fill_counter in range(0,4*(len(variable_names)-1)+1,4):
    
        MI_table_var_changes.iloc[fill_counter,:] = [spacing]*13
        
        MI_table_var_changes.iloc[fill_counter+1,:] = [optimal_variable_value[int(fill_counter/4)]-1, 
                                                       optimal_variable_value[int(fill_counter/4)],
                                                       optimal_variable_value[int(fill_counter/4)]+1,
                                                       spacing,
                                                       0,
                                                       round(optimal_variable_value[int(fill_counter/4)]*0.5),
                                                       round(optimal_variable_value[int(fill_counter/4)]*0.75),
                                                       round(optimal_variable_value[int(fill_counter/4)]*0.9),
                                                       optimal_variable_value[int(fill_counter/4)],
                                                       round(optimal_variable_value[int(fill_counter/4)]*1.1),
                                                       round(optimal_variable_value[int(fill_counter/4)]*1.25),
                                                       round(optimal_variable_value[int(fill_counter/4)]*1.5),
                                                       round(optimal_variable_value[int(fill_counter/4)]*2)] 
        
        MI_table_var_changes.iloc[fill_counter+2,:] = [optimal_objective_value-obj_coef[int(fill_counter/4)],
                                                       optimal_objective_value, 
                                                       optimal_objective_value+obj_coef[int(fill_counter/4)], 
                                                       spacing, 
                                                       optimal_objective_value-obj_coef[int(fill_counter/4)]*round(optimal_variable_value[int(fill_counter/4)]*(-1)),
                                                       optimal_objective_value-obj_coef[int(fill_counter/4)]*round(optimal_variable_value[int(fill_counter/4)]*(-0.5)),
                                                       optimal_objective_value-obj_coef[int(fill_counter/4)]*round(optimal_variable_value[int(fill_counter/4)]*(-0.25)),
                                                       optimal_objective_value-obj_coef[int(fill_counter/4)]*round(optimal_variable_value[int(fill_counter/4)]*(-0.1)),
                                                       optimal_objective_value, 
                                                       optimal_objective_value-obj_coef[int(fill_counter/4)]*round(optimal_variable_value[int(fill_counter/4)]*(0.1)),
                                                       optimal_objective_value-obj_coef[int(fill_counter/4)]*round(optimal_variable_value[int(fill_counter/4)]*(0.25)),
                                                       optimal_objective_value-obj_coef[int(fill_counter/4)]*round(optimal_variable_value[int(fill_counter/4)]*(0.5)), 
        ...
        MI_table_var_changes.iloc[fill_counter+3,0] = violated_constraint_minus[int(fill_counter/4)]
        MI_table_var_changes.iloc[fill_counter+3,1] = None
        MI_table_var_changes.iloc[fill_counter+3,2] = violated_constraint_plus[int(fill_counter/4)]
        MI_table_var_changes.iloc[fill_counter+3,3] = spacing
        MI_table_var_changes.iloc[fill_counter+3,4] = violated_constraint_m100[int(fill_counter/4)]
        MI_table_var_changes.iloc[fill_counter+3,5] = violated_constraint_m50[int(fill_counter/4)]
        MI_table_var_changes.iloc[fill_counter+3,6] = violated_constraint_m25[int(fill_counter/4)]
        MI_table_var_changes.iloc[fill_counter+3,7] = violated_constraint_m10[int(fill_counter/4)]
        MI_table_var_changes.iloc[fill_counter+3,8] = None
        MI_table_var_changes.iloc[fill_counter+3,9] = violated_constraint_p10[int(fill_counter/4)]
        MI_table_var_changes.iloc[fill_counter+3,10] = violated_constraint_p25[int(fill_counter/4)]
        MI_table_var_changes.iloc[fill_counter+3,11] = violated_constraint_p50[int(fill_counter/4)]
        MI_table_var_changes.iloc[fill_counter+3,12] = violated_constraint_p100[int(fill_counter/4)]
        ...
        for negative_counter in range(13):
            ...
            try:
                if MI_table_var_changes.iloc[fill_counter+1,negative_counter] < 0:
                    MI_table_var_changes.iloc[fill_counter+2,negative_counter] = 'INFESIBLE'
                    MI_table_var_changes.iloc[fill_counter+3,negative_counter] = variable_names[int(fill_counter/4)] + ' >= 0'
                    MI_table_var_changes.iloc[fill_counter+1,negative_counter] = '< 0'
...
def MI_Obj_Coef(obj_coef, optimal_solution_Branch_and_Bound):
    
    global MI_table_obj_coef
    
    original_obj_coef = copy.deepcopy(obj_coef[:])
    
    original_optimal_solution = copy.deepcopy(optimal_solution_Branch_and_Bound[:])
    ...
    for coef_counter in range(len(variable_names)):
        ...
        obj_minus = optimal_solution_Branch_and_Bound[0]
        ...
        Simplex()
        Branch_And_Bound(optimal_solution_Simplex)
        obj_plus = optimal_solution_Branch_and_Bound[0]
        ...
        Simplex() 
        Branch_And_Bound(optimal_solution_Simplex)
        obj_m100 = optimal_solution_Branch_And_Bound[0]  
        ...
        Simplex() 
        Branch_And_Bound(optimal_solution_Simplex)
        obj_m50 = optimal_solution_Branch_And_Bound[0]  
        ...
        Simplex()
        Branch_And_Bound(optimal_solution_Simplex)
        obj_m25 = optimal_solution_Branch_And_Bound[0] 
        ...
        Simplex()
        Branch_And_Bound(optimal_solution_Simplex)
        obj_m10 = optimal_solution_Branch_And_Bound[0]  
        ...
        Simplex() 
        Branch_And_Bound(optimal_solution_Simplex)
        obj_p10 = optimal_solution_Branch_And_Bound[0]   
        ...
        Simplex() 
        Branch_And_Bound(optimal_solution_Simplex)
        obj_p25 = optimal_solution_Branch_And_Bound[0]     
        ...
        Simplex()
        Branch_And_Bound(optimal_solution_Simplex)
        obj_p50 = optimal_solution_Branch_And_Bound[0]      
        ...
        Simplex()
        Branch_And_Bound(optimal_solution_Simplex)
        obj_p100 = optimal_solution_Branch_And_Bound[0]  
    ...
    MI_table_obj_coef = pd.DataFrame(columns = col_obj_coef, index = row_obj_coef).fillna('')
    
    fill_counter = 0
    
    for fill_counter in range(0,4*(len(variable_names)-1)+1,4):
    
        MI_table_obj_coef.iloc[fill_counter,:] = [spacing]*13
        
        MI_table_obj_coef.iloc[fill_counter+1,:] = [original_obj_coef[int(fill_counter/4)]-1, 
                                                    original_obj_coef[int(fill_counter/4)], 
                                                    original_obj_coef[int(fill_counter/4)]+1, 
                                                    spacing, 
                                                    0, 
        ...
        MI_table_obj_coef.iloc[fill_counter+2,:] = [new_optimal_minus[int(fill_counter/4)], 
                                                    original_optimal_solution[0], 
                                                    new_optimal_plus[int(fill_counter/4)], 
                                                    spacing, 
                                                    new_optimal_m100[int(fill_counter/4)], 
        ...
        for infeasible_counter in range(13):
            if MI_table_obj_coef.iloc[fill_counter+2,infeasible_counter] == 'INFESIBLE':
                MI_table_obj_coef.iloc[fill_counter+3,infeasible_counter] = 'INFESIBLE'
            else:
                MI_table_obj_coef.iloc[fill_counter+3,0] = MI_table_obj_coef.iloc[fill_counter+2,0] - MI_table_obj_coef.iloc[fill_counter+2,1]
                MI_table_obj_coef.iloc[fill_counter+3,1] = None
                MI_table_obj_coef.iloc[fill_counter+3,2] = MI_table_obj_coef.iloc[fill_counter+2,2] - MI_table_obj_coef.iloc[fill_counter+2,1]
                MI_table_obj_coef.iloc[fill_counter+3,3] = spacing
                
                for perc_fill_counter in range(4,13):
                    MI_table_obj_coef.iloc[fill_counter+3,perc_fill_counter] = MI_table_obj_coef.iloc[fill_counter+2,perc_fill_counter] - MI_table_obj_coef.iloc[fill_counter+2,1]
                    perc_fill_counter += 1
                                              
                MI_table_obj_coef.iloc[fill_counter+3,8] = None

    
    return MI_table_obj_coef
...
def MI_Con_Bound(bound, constraint_names, optimal_solution_Branch_and_Bound): 
    
    global MI_table_bound
    ...
    for bound_counter in range(len(constraint_names)):
        ...
        Simplex()  
        Branch_And_Bound(optimal_solution_Simplex)
        bound_minus = optimal_solution_Branch_And_Bound[0] 
        ...
        Simplex() 
        Branch_And_Bound(optimal_solution_Simplex)
        bound_plus = optimal_solution_Branch_And_Bound[0] 
        ...
        Simplex() 
        Branch_And_Bound(optimal_solution_Simplex)
        bound_m100 = optimal_solution_Branch_And_Bound[0]   
        ...
        Simplex() 
        Branch_And_Bound(optimal_solution_Simplex)
        bound_m50 = optimal_solution_Branch_And_Bound[0]   
        ...
        Simplex()
        Branch_And_Bound(optimal_solution_Simplex)
        bound_m25 = optimal_solution_Branch_And_Bound[0]    
        ...
        Simplex()
        Branch_And_Bound(optimal_solution_Simplex)
        bound_m10 = optimal_solution_Branch_And_Bound[0]  
        ...
        Simplex()
        Branch_And_Bound(optimal_solution_Simplex)
        bound_p10 = optimal_solution_Branch_And_Bound[0]   
        ...
        Simplex()
        Branch_And_Bound(optimal_solution_Simplex)
        bound_p25 = optimal_solution_Branch_And_Bound[0]  
        ...
        Simplex()
        Branch_And_Bound(optimal_solution_Simplex)
        bound_p50 = optimal_solution_Branch_And_Bound[0]     
        ...
        Simplex()
        Branch_And_Bound(optimal_solution_Simplex)
        bound_p100 = optimal_solution_Branch_And_Bound[0]   
    ...
    MI_table_bound = pd.DataFrame(columns = col_bound, index = row_bound).fillna('')
    
    fill_counter = 0
    
    for fill_counter in range(0,4*(len(constraint_names)-1)+1,4):
    
        MI_table_bound.iloc[fill_counter,:] = [spacing]*13
        
        MI_table_bound.iloc[fill_counter+1,:] = [original_con_bound[int(fill_counter/4)]-1, 
                                                 original_con_bound[int(fill_counter/4)], 
                                                 original_con_bound[int(fill_counter/4)]+1, 
                                                 spacing, 
                                                 0, 
        ...
        MI_table_bound.iloc[fill_counter+2,:] = [new_optimal_minus[int(fill_counter/4)], 
                                                 original_optimal_solution[0], 
                                                 new_optimal_plus[int(fill_counter/4)], 
                                                 spacing, 
                                                 new_optimal_m100[int(fill_counter/4)], 
        ...
        for infeasible_counter in range(13):
            if MI_table_bound.iloc[fill_counter+2,infeasible_counter] == 'INFESIBLE':
                MI_table_bound.iloc[fill_counter+3,infeasible_counter] = 'INFESIBLE'                                     
                                              
            else:
                MI_table_bound.iloc[fill_counter+3,0] = MI_table_bound.iloc[fill_counter+2,0] - MI_table_bound.iloc[fill_counter+2,1]
                MI_table_bound.iloc[fill_counter+3,1] = None
                MI_table_bound.iloc[fill_counter+3,2] = MI_table_bound.iloc[fill_counter+2,2] - MI_table_bound.iloc[fill_counter+2,1]
                MI_table_bound.iloc[fill_counter+3,3] = spacing
                                              
                for perc_fill_counter in range(4,13):
                    MI_table_bound.iloc[fill_counter+3,perc_fill_counter] = MI_table_bound.iloc[fill_counter+2,perc_fill_counter] - MI_table_bound.iloc[fill_counter+2,1]
                    perc_fill_counter += 1
                                              
                MI_table_bound.iloc[fill_counter+3,8] = None

        # constraint bound less than 0 is not making sense
        
        if MI_table_bound.iloc[fill_counter+1,0] < 0:
            MI_table_bound.iloc[fill_counter+1,0] = 'INFESIBLE'
            MI_table_bound.iloc[fill_counter+2,0] = 'INFESIBLE'
            MI_table_bound.iloc[fill_counter+3,0] = 'INFESIBLE'
                
    
    return MI_table_bound
    
    
    
# constraint remove sensitivity analysis function

def MI_Con_Remove(constraint_names, constraint, constraint_type, bound, optimal_solution_Branch_and_Bound):
    
    global MI_table_con_remove
    ...
    if len(soft_con_index) == 0:
        ...
        MI_table_con_remove = None
        print('There are no soft constraints to be removed.')
        
        return MI_table_con_remove
        ...
        for soft_con_remove_counter in soft_con_index:
            ...
            Simplex()
            Branch_And_Bound(optimal_solution_Simplex)
            Simplex_excl_soft = optimal_solution_Branch_And_Bound[0]
        ...
        MI_table_con_remove = pd.DataFrame(columns = col_con_remove, index = row_con_remove).fillna('')

        fill_counter = 0

        for fill_counter in range(0,4*(len(soft_con_index)-1)+1,4):

            MI_table_con_remove.iloc[fill_counter,:] = [spacing]*2
            MI_table_con_remove.iloc[fill_counter+1,:] = ' '
            MI_table_con_remove.iloc[fill_counter+2,:] = [original_optimal_solution[0], optimal_excl_soft[int(fill_counter/4)]]

            infeasible_counter = 0

            for infeasible_counter in range(2):
                if MI_table_con_remove.iloc[fill_counter+2,infeasible_counter] == 'INFESIBLE':
                    MI_table_con_remove.iloc[fill_counter+3,infeasible_counter] = 'INFESIBLE'
                else:
                    MI_table_con_remove.iloc[fill_counter+3,0] = None
                    MI_table_con_remove.iloc[fill_counter+3,1] = MI_table_con_remove.iloc[fill_counter+2,1] - MI_table_con_remove.iloc[fill_counter+2,0]
...
def MI_Sensitivity_Analysis():
    ...
    MI_SA_Opt_Var = MI_Optimal_Var(optimal_solution_Branch_and_Bound)
    display(MI_SA_Opt_Var)
    
    print('\nThe table above shows that the best value for ' + str(obj_names[0]) + ' is ' + str(MI_SA_Opt_Var.iloc[2,1]) + ', under the optimal solution, ')
    print('when the ' + str(len(variable_names)) + ' variables: ' + str(variable_names[:]) + ' , are set to: ')
    
    var_values_counter = 0
    for var_values_counter in range(len(variable_names)):
        print(str(variable_names[var_values_counter]) + ' = ' + str(MI_SA_Opt_Var.iloc[var_values_counter*4 + 1, 1]) + ' , ')
        var_values_counter += 1
    print('respectively.\n')
    
    print('If variables are set to other values, the objective value ' + str(obj_names[0]) + ' will be either inferior than ' + str(MI_SA_Opt_Var.iloc[2,1]) + ' , or violating constraints! \n')
    print('The following graphs have shown the impacts on the objective value by changing the optimal vaiables value: \n')
    print('(Notes: Blue points are feasible, while Red points are violating at least one constraints)\n')
    ...
    for plot_counter in range(len(variable_names)):
        ...
        for color_counter in range(MI_SA_Opt_Var.shape[1]):
            if isinstance(MI_SA_Opt_Var.iloc[plot_counter*4 + 1, color_counter], int) == True or isinstance(MI_SA_Opt_Var.iloc[plot_counter*4 + 1, color_counter], float) == True:
                if isinstance(MI_SA_Opt_Var.iloc[plot_counter*4 + 2, color_counter], int) == True or isinstance(MI_SA_Opt_Var.iloc[plot_counter*4 + 2, color_counter], float) == True:
                    if MI_SA_Opt_Var.iloc[plot_counter*4 + 3, color_counter] == None:
                        x_b.append(MI_SA_Opt_Var.iloc[plot_counter*4 + 1, color_counter])
                        y_b.append(MI_SA_Opt_Var.iloc[plot_counter*4 + 2, color_counter])
                    else:
                        x_r.append(MI_SA_Opt_Var.iloc[plot_counter*4 + 1, color_counter])
                        y_r.append(MI_SA_Opt_Var.iloc[plot_counter*4 + 2, color_counter])
    ...
    MI_SA_Obj_Coe = MI_Obj_Coef(obj_coef, optimal_solution_Branch_and_Bound)
    display(MI_SA_Obj_Coe)
    
    # return MI_table_obj_coef
    
    print('\nThe table above shows that the best value for ' + str(obj_names[0]) + ' is ' + str(MI_SA_Obj_Coe.iloc[2,1]) + ', under the optimal solution, ')
    print('when the coefficients of ' + str(len(variable_names)) + ' variables: ' + str(variable_names[:]) + ' , are set to: ')
    
    obj_coe_counter = 0
    row_name_list = list(MI_SA_Obj_Coe.index)
    for obj_coe_counter in range(len(variable_names)):
        print(str(row_name_list[obj_coe_counter*4 + 1]) + ' = ' + str(MI_SA_Obj_Coe.iloc[obj_coe_counter*4 + 1, 1]) + ' , ')
        obj_coe_counter += 1
    print('respectively.\n')
    
    print('If coefficients are set to other values, the objective value ' + str(obj_names[0]) + ' might be changed and different from ' + str(MI_SA_Obj_Coe.iloc[2,1]) + ' , or even infeasible.\n')
    print('The following graphs have shown the impacts on the objective value by changing the coefficient of each variable: \n')
    ...
    for plot_counter in range(len(variable_names)):
        ...
        for col_counter in range(MI_SA_Obj_Coe.shape[1]):
            if isinstance(MI_SA_Obj_Coe.iloc[plot_counter*4 + 1, col_counter], int) == True or isinstance(MI_SA_Obj_Coe.iloc[plot_counter*4 + 1, col_counter], float) == True:
                if isinstance(MI_SA_Obj_Coe.iloc[plot_counter*4 + 2, col_counter], int) == True or isinstance(MI_SA_Obj_Coe.iloc[plot_counter*4 + 2, col_counter], float) == True:
                    x_k.append(MI_SA_Obj_Coe.iloc[plot_counter*4 + 1, col_counter])
                    y_k.append(MI_SA_Obj_Coe.iloc[plot_counter*4 + 2, col_counter])
    ...
    MI_SA_Con_Bou = MI_Con_Bound(bound, constraint_names, optimal_solution_Branch_and_Bound)
    display(SA_Con_Bou)
    
    # return table_bound
    
    print('\nThe table above shows that the best value for ' + str(obj_names[0]) + ' is ' + str(MI_SA_Con_Bou.iloc[2,1]) + ', under the optimal solution, ')
    print('when the constraint bound of ' + str(len(constraint_names)) + ' constraints: ' + str(constraint_names[:]) + ' , are set to: ')
    
    con_bou_counter = 0
    row_name_list = list(MI_SA_Con_Bou.index)
    for con_bou_counter in range(len(constraint_names)):
        print(str(row_name_list[con_bou_counter*4 + 1]) + ' = ' + str(MI_SA_Con_Bou.iloc[con_bou_counter*4 + 1, 1]) + ' , ')
        con_bou_counter += 1
    print('respectively.\n')
    
    print('If bounds are set to other values, the objective value ' + str(obj_names[0]) + ' might be changed and different from ' + str(MI_SA_Con_Bou.iloc[2,1]) + ' .\n')
    
    print('The slack value shows that how much each constraint can be changed before it will be violated.\n')
    slack_optimal_solution = copy.deepcopy(optimal_solution_Branch_and_Bound[2])
    print('The slack value for constraints ' + str(constraint_names[:]) + ' are ' + str(slack_optimal_solution[:]) + ' , respectively.\n')
    print('Meaning that: \n')
    slack_counter = 0
    for slack_counter in range(len(constraint_names)):
    ...
    for plot_counter in range(len(constraint_names)):
        ...
        for col_counter in range(MI_SA_Con_Bou.shape[1]):
            if isinstance(MI_SA_Con_Bou.iloc[plot_counter*4 + 1, col_counter], int) == True or isinstance(MI_SA_Con_Bou.iloc[plot_counter*4 + 1, col_counter], float) == True:
                if isinstance(MI_SA_Con_Bou.iloc[plot_counter*4 + 2, col_counter], int) == True or isinstance(MI_SA_Con_Bou.iloc[plot_counter*4 + 2, col_counter], float) == True:
                    x_g.append(MI_SA_Con_Bou.iloc[plot_counter*4 + 1, col_counter])
                    y_g.append(MI_SA_Con_Bou.iloc[plot_counter*4 + 2, col_counter])
    ...
    MI_SA_Con_Rem = MI_Con_Remove(constraint_names, constraint, constraint_type, bound, optimal_solution_Branch_and_Bound)
    
    # return MI_table_con_remove, None if there is no soft constraints
    
    try:
        shape_checker = MI_SA_Con_Rem.shape[0]
        display(SA_Con_Rem)
        ...
        for exp_counter in range(len(soft_cons_index)):
            if MI_SA_Con_Rem.iloc[exp_counter*4 + 2, 1] == 'INFEASIBLE':
                print('If soft constraint ' + str(soft_cons[exp_counter]) + ' is removed, the objective value ' + str(obj_names[0]) + ' will become infeasible.\n')
            elif MI_SA_Con_Rem.iloc[exp_counter*4 + 3, 1] == 0:
                print('If soft constraint ' + str(soft_cons[exp_counter]) + ' is removed, the objective value ' + str(obj_names[0]) + ' is unchanged.\n')
            else:
                print('If soft constraint ' + str(soft_cons[exp_counter]) + ' is removed, the objective value ' + str(obj_names[0]) + ' is changed to ' + str(MI_SA_Con_Rem.iloc[exp_counter*4 + 2, 1]) + ' .\n')
...
# MI_Sensitivity_Analysis function for MILP, otherwise Sensitivity_Analysis function:

try: 
    MI_checker = copy.deepcopy(Deep_Copy_integer_constraint)
except NameError:
